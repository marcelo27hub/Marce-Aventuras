# pierde_juego.py  — versión simple
...
def _build_animaciones(escala=(180, 180)):
    ...
    anims = []
    for i in range(1, 7):
        a = _load_or_placeholder(f"movimiento_triste{2*i-1}.png", escala)
        b = _load_or_placeholder(f"movimiento_triste{2*i}.png",   escala)
        anims.append([a, b])
    return anims

def Pantala_final_perdio(
    ...
    *,
    ...
    # Textos
    titulo   = font_big.render("GAME OVER", True, (255,255,255))
    subtitulo= font_big.render("Marce no pudo llegar a Code Pro", True, (255,255,255))
    sep = 20
